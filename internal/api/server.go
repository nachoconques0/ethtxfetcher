...
type Server struct {
	*http.Server
	parserServer parserCtrl
}

func NewServer(port string, parserSrv parserCtrl) (*Server, error) {
	if port == "" {
...
func (s *Server) router() *http.ServeMux {
	...
	router.HandleFunc("GET /block", s.parserServer.GetCurrentBlock)
	router.HandleFunc("POST /subscribe/{address}", s.parserServer.Subscribe)
	router.HandleFunc("GET /address/{address}/tx", s.parserServer.GetTransactions)
	return router
}
